...
| ▶️ 12.5 Vigex para Windows — instalador Docker | **R-099** — Experiencia 100% Windows sin tocar Linux: `VigexSetup.exe` instala Docker Engine silenciosamente, descarga la imagen `vigex/panel`, registra el servicio Windows y abre el navegador en `localhost:8000`. El cliente nunca ve una terminal ni configura Linux. Alcance: (1) `Dockerfile` del panel, (2) `docker-compose.yml` con volúmenes persistentes, (3) instalador PowerShell/NSIS, (4) icono de bandeja + script de actualización `vigex-update.bat`. Los servidores gestionados pueden seguir siendo Linux — el panel corre en Windows. ▶️ **Planificado (siguiente fase de producto)** | ▶️ |
...
| 2026-06-12 | **R-099 planificado — Vigex para Windows (instalador Docker).** Objetivo: cliente Windows instala `VigexSetup.exe` y Vigex aparece en el navegador sin tocar Linux. Docker Engine como capa de virtualización invisible. Alcance: Dockerfile + docker-compose + instalador NSIS/PowerShell + bandeja del sistema + actualizador. Motivación: ~65% de SMEs europeas usan Windows internamente; eliminar la barrera del VPS Linux amplía el mercado objetivo significativamente. | Decisión: Docker es el estándar industrial para self-hosted (Portainer, Netdata, Nextcloud, Bitwarden); el diferenciador es el instalador, no la tecnología. |
