...
Este documento define cómo validar que el instalador base de Vigex cumple el criterio de idempotencia.
...
- Crea la estructura en `/opt/vigex/api`.
- Crea entorno virtual.
- Instala dependencias Python.
- Crea el servicio `vigex-api`.
...
- No debe fallar porque `/opt/vigex/api` ya existe.
...
- El `WorkingDirectory` apunta a `/opt/vigex/api`.
...
ls -ld /opt/vigex
ls -ld /opt/vigex/api
ls -ld /opt/vigex/api/venv
ls -l /opt/vigex/api/main.py
ls -l /opt/vigex/api/requirements.txt
...
Máquina con Vigex ya instalado.
...
Máquina donde existe `/opt/vigex/api`, pero falta el entorno virtual o el servicio.
...
Máquina donde ya existe `/etc/systemd/system/vigex-api.service`.
...
- Revisión del script `install_vigex_api.sh`.
